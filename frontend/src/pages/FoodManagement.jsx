    ...
    <>
      ...
      <div className="p-8">
        ...
        <div className="mb-4 flex items-center gap-4">
          <input
            type="text"
            className="border rounded-3xl px-3 py-2 w-full max-w-xs"
              ...
              {foods.filter(f => {
                ...
                <tr key={f._id} className="text-center">
                  ...
                  <td className="py-2 px-4 border">
                    <button className="bg-yellow-500 text-white px-3 py-1 rounded-3xl hover:bg-yellow-600 mr-2" onClick={() => openEditModal(f)} disabled={loading}>Edit</button>
                    <button className="bg-red-600 text-white px-3 py-1 rounded-3xl hover:bg-red-700" onClick={() => handleDelete(f._id)} disabled={loading}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {showModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-gray-900 p-6 rounded-2xl shadow w-full max-w-sm">
              <h2 className="text-xl font-bold mb-4">{editId ? "Edit Food Item" : "Add Food Item"}</h2>
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div>
                  <label className="block mb-1 font-medium">Name</label>
                  <input name="name" value={form.name} onChange={handleChange} className="w-full px-3 py-2 border rounded-3xl" required />
                </div>
                <div>
                  <label className="block mb-1 font-medium">Price</label>
                  <input name="price" type="number" value={form.price} onChange={handleChange} className="w-full px-3 py-2 border rounded-3xl" required min="0" />
                </div>
                <div>
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-3xl" disabled={loading}>{editId ? "Update" : "Add"}</button>
                </div>
              </form>
            </div>
          </div>
